...
export async function getAppOnlyBearerToken() {
  ...
  const { data } = await axios.post(`${graphConfig.tokenEndpoint}`, requestBody)
  return {
...
export async function uploadFileToSharePointAsync(file: File, fileName: string, bearerToken: string, storageName: 'security' | 'scg') {
  try {
    ...
    const documentName = storageName === 'security' ? 'Security Application' : 'SCG Applications'
...
export async function saveToSharePointAsync(form: FormType, bearerToken: string) {
  try {
    const client = Client.init({
      authProvider: (done) => {
        done(null, bearerToken);
    ...
    const itemPayload: any = {
      fields: {
        ...
        authorization_effective_date: form.authorization?.effectiveDate ? new Date(form.authorization?.effectiveDate).toISOString() : undefined,
    ...
    if (form.account?.invoiceDelivery) {
      const keys = Object.keys(form.account.invoiceDelivery) as Array<keyof typeof form.account.invoiceDelivery>
      itemPayload.fields = {
        ...itemPayload.fields,
        "invoice_delivery@odata.type": "Collection(Edm.String)",
        invoice_delivery: keys.filter((key) => form.account?.invoiceDelivery?.[key])
      }
    }

    if (form.account?.paymentMethod) {
      const keys = Object.keys(form.account.paymentMethod) as Array<keyof typeof form.account.paymentMethod>
      itemPayload.fields = {
        ...itemPayload.fields,
        "payment_method@odata.type": "Collection(Edm.String)",
        payment_method: keys.filter((key) => form.account?.paymentMethod?.[key])
      }
    }

    if (form.emergency?.unreachable) {
      const keys = Object.keys(form.emergency.unreachable) as Array<keyof typeof form.emergency.unreachable>
      itemPayload.fields = {
        ...itemPayload.fields,
        "emergency_unreachable@odata.type": "Collection(Edm.String)",
        emergency_unreachable: keys.filter((key) => form.emergency?.unreachable?.[key] ? key.replace(/_/g, ' ') : false)
      }
    }
...
export async function getSharePointData(bearerToken: string, itemId: string) {
  try {
    ...
    const responseItem = await client.api(`/sites/${graphConfig.gfhpmGraphSiteId}/lists/${graphConfig.listId}/items/${itemId}`).get();
